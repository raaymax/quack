...
export class Cache<T extends any[]> {
  ...
  cleanup() {
    this.repo = this.repo.filter((entry) =>
      entry.timestamp > new Date().getTime() - 1000 * 60 * 60
    );
  }
  ...
  get(q: CacheQuery): CacheEntry<T> | null {
    ...
    if (
      "from" in q && "to" in q && q.from !== undefined && q.to !== undefined
    ) {
      const { from, to } = q;
      return repo.find((entry) =>
        entry.containsEntirely(new Range(from, to))
      ) || null;
    }
