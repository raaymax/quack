...
export const Login = ({ children }: LoginProps) => {
  ...
  const validate = useCallback(() =>
    client.api.auth.restoreSession()
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener("hashchange", reload);
    return () => globalThis.removeEventListener("hashchange", reload);
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const value = Object.fromEntries(fd.entries()) as {
      email: string;
      password: string;
    };
    ...
    if (ret.status === "ok") {
      ...
      if (ret.errorCode === "PASSWORD_RESET_REQUIRED") {
        setMsg(
          <div>
            {ret.message}. <a href={`/#/reset/${ret.token}`}>reset password</a>
          </div>,
        );
      } else {
