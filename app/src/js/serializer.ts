...
export const fromDom = (
  dom: HTMLElement,
): MessageToSend | CommandToSend | MessageToUpdate => {
...
function is<T extends MessageBodyPart>(
  data: MessageBodyPart,
  key: string,
): data is T {
...
const mapNodes = (dom: HTMLElement, info: SerializeInfo): MessageBody => (
  !dom.childNodes
    ? []
    : ([...dom.childNodes] as HTMLElement[]).map((n): MessageBody => {
      ...
      if (n.nodeName === "A") {
        console.log("link", n, {
          link: mapNodes(n, info),
          _href: n.getAttribute("href") ?? "",
        });
      ...
      if (n.nodeName === "IMG") {
        return {
          ...
          _alt: n.getAttribute("alt") ?? "",
        };
      }
      if (n.nodeName === "SPAN" && n.className === "emoji") {
        return { emoji: n.getAttribute("emoji") ?? "" };
      }
      if (n.nodeName === "SPAN" && n.className === "channel") {
        return { channel: n.getAttribute("channelId") ?? "" };
      }
